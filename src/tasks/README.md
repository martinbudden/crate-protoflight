...
Protoflight has four realtime tasks (`imu`, `pid_gyro`, `motor_mixer`, and `rx`) and a varying number of other tasks,
...
| ------ | -------------------- | -------------------------------------------------------------- | ---------- |
| Core 1 | Dedicated Executor   | `gyro_pid`, `imu` (8kHz)                                       | Real-time  |
| Core 0 | Interrupt Executor   | `motor_mixer` (8kHz*),<br> `rx` (~50Hz to 500Hz)               | High       |
...
| GPS_PUB_SUB_CHANNEL          | PubSub | GpsMessage           | 4               | gps             | autopilot, blackbox, msp, osd |
